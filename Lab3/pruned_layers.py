...
class PruneLinear(nn.Module):
    ...
    def prune_by_percentage(self, q=5.0):
        ...
        all_weight = self.linear.weight.data.abs().cpu().detach().numpy().flatten()
        threshold = np.percentile(all_weight, q)

        self.mask = self.linear.weight.data.abs() > threshold
        self.mask = self.mask.float()
        
        self.linear.weight.data.mul_(self.mask)

        self.sparsity = np.count_nonzero(self.linear.weight.data.cpu().detach().numpy()==0) / len(all_weight)
    ...
    def prune_by_std(self, s=0.25):
        ...
        all_weight = self.linear.weight.data.cpu().detach().numpy().flatten()
        threshold = np.std(all_weight) * s
...
class PrunedConv(nn.Module):
    ...
    def prune_by_percentage(self, q=5.0):
        ...
        all_weight = self.conv.weight.data.abs().cpu().detach().numpy().flatten()
        threshold = np.percentile(all_weight, q)

        self.mask = self.conv.weight.data.abs() > threshold
        self.mask = self.mask.float()
        
        self.conv.weight.data.mul_(self.mask)

        self.sparsity = np.count_nonzero(self.conv.weight.data.cpu().detach().numpy()==0) / len(all_weight)
    ...
    def prune_by_std(self, s=0.25):
        ...
        all_weight = self.conv.weight.data.cpu().detach().numpy().flatten()
        threshold = np.std(all_weight) * s
